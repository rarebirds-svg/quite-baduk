...
async def read_session(
    sess: CurrentSession, response: Response, db: DbSession
) -> SessionPublic:
    ...
    if sess.account_id is not None:
        acc = (
            await db.execute(select(Account).where(Account.id == sess.account_id))
        ).scalar_one_or_none()
        if acc is not None:
            account = AccountPublic(provider=acc.provider, email=acc.email)
    return SessionPublic(id=sess.id, nickname=sess.nickname, account=account)
