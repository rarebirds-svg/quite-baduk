...
async def prune_old_visits(db: AsyncSession, days: int = DEFAULT_RETENTION_DAYS) -> int:
    ...
    같은 컷오프로 analytics_salts의 옛 솔트도 함께 지운다 — 방문 행이 사라지면
    그날의 솔트는 더 쓸 데가 없다. 반환값은 visit_hits 삭제 건수만 센다.
    """
    cutoff = datetime.now(UTC).replace(tzinfo=None) - timedelta(days=days)
    result = await db.execute(delete(VisitHit).where(VisitHit.created_at < cutoff))
    await db.execute(delete(AnalyticsSalt).where(AnalyticsSalt.day < cutoff.strftime("%Y-%m-%d")))
    await db.commit()
