...
### AP-20260920-01
- 액션: PR [#102](https://github.com/rarebirds-svg/quite-baduk/pull/102) 머지 → prod `git pull --ff-only` → `com.baduk.api` 재기동 (🟡 `main` 머지 + prod 재기동)
- 근거: 이슈 #101 — 사용자 기권(`resign_game`)이 `finished_at`을 기록하지 않아 기권 대국 99건이 `status='finished'`인데 종료 시각 NULL로 남는다(9/19 09:00 DB 실측으로 발굴). 9/20 04:30 dev-cycle이 TDD로 수정(`game_service.py` `resign_game`에 `finished_at = now(UTC)` 2줄 추가, 다른 종료 경로 4곳과 동일 패턴) + 회귀 테스트 1개 추가해 PR 생성. `pytest` 610 passed·`ruff`·`mypy strict` clean, Fable 리뷰 APPROVE, CI 4잡(backend·frontend·app-shell-build·e2e) SUCCESS, `MERGEABLE`/`CLEAN`, `merge-tree` 충돌 0, 변경 파일 2개(+24/−0), 마이그레이션 없음. PR #98·#100과 파일 겹침 — `game_service.py`는 #98과 같은 파일이나 `merge-tree`로 충돌 0 확인.
- 영향: 백엔드 서비스 코드 변경이라 재기동 전까지 라이브 미반영. 발효 후 기권 대국부터 `finished_at`이 채워지며 `admin.py` 일별 종료 집계에 기권 대국이 포함되기 시작한다. **기존 99건 백필은 PR 범위 밖**(`max(moves.played_at)` 근사로 가능, 사람 판단). `prio:low`라 AP-20260916-01(api 재기동)과 한 회차로 묶어 발효 권장 — 세 PR을 함께 머지하면 api 재기동 1회·web 재기동 1회로 끝난다.
- 실행 절차: (1) `gh pr checks 102` 재확인 → (2) `gh pr merge 102 --squash --delete-branch` → (3) `cd /Users/daegong/projects/baduk && git pull --ff-only` → (4) `sqlite3` `moves.played_at` 최근 5분 내 착수 없음 확인 → (5) `launchctl kickstart -k gui/$(id -u)/com.baduk.api` → (6) `/api/health` 200·`migrations.pending false` 확인 → (7) 가능하면 기권 1회 후 `finished_at` non-null 확인 → (8) `state/log/`에 기록.
- 상태: 대기 (2026-09-20 09:00 등재, 9/20 21:00 재확인 — 12시간째 · 1회 재확인. PR #102 CI 4잡 SUCCESS·`MERGEABLE`/`CLEAN`·`merge-tree` 충돌 0 재확인(main보다 2커밋 뒤). **9/20 21:00 기준 #513·#514 진행 중(마지막 착수 20:57 KST)이라 지금은 api 재기동 부적기** — 실행 시 절차 (4) 5분 착수 확인 필수)
...
- 상태: 대기 (2026-09-16 09:00 등재, 9/16 21:00·9/17 09:00·9/17 21:00·9/18 09:00·9/18 21:00·9/19 09:00·9/19 21:00·9/20 09:00·9/20 21:00 재확인 — 108시간째 · 9회 재확인. **9/20 21:00 실측 — 세션 33이 13路 대국 8판(492수)을 두고 #513·#514가 진행 중(마지막 착수 20:57 KST)이라 지금은 재기동 부적기. 09:00의 "재기동 적기" 판단은 철회, 심야·새벽 회차 권장.** (9/20 09:00 기록) PR #98 CI 4잡 SUCCESS·`MERGEABLE`/`CLEAN`·`merge-tree` 충돌 0 재확인(main보다 10커밋 뒤, ops 문서·콘텐츠뿐). **9/19 11:06 KST 라이브 재현 2회차 — 세션 34가 13路 2점 치석 대국 생성 3회 연속 500(재시도 전부 실패, 고아 `active` #502~#504), 치석 포기 후 호선으로 우회. 긴급도 상승.** 9/20 09:00 기준 진행 중 대국 0(마지막 착수 9/19 11:44 KST #506, 이후 21시간 무착수·밤사이 신규 대국 0)이라 재기동 적기 — 실행 시 절차 (4) 최근 5분 착수 확인만 거칠 것. AP-20260917-01·AP-20260920-01과 한 회차로 묶어 발효 권장)
...
- 상태: 대기 (2026-09-17 09:00 등재, 9/17 21:00·9/18 09:00·9/18 21:00·9/19 09:00·9/19 21:00·9/20 09:00·9/20 21:00 재확인 — 84시간째 · 7회 재확인. 9/20 21:00 `[object Object]` 9회 추가 재현(SSR 8 + 외부 1, 누적 684). web 재기동은 대국 WS와 무관하나 api 재기동과 묶는다면 #513·#514 진행 중이라 지금은 부적기. (9/20 09:00 기록) PR #100 CI 4잡 SUCCESS·`MERGEABLE`/`CLEAN`·`merge-tree` 충돌 0 재확인(main보다 8커밋 뒤), 9/19 21:00 이후 `[object Object]` 질의 10회 추가 재현(SSR 7 + 외부 3, 누적 675 실측))
