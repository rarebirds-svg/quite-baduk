...
### AP-20260830-01 — PR #80 머지: 헤드리스 claude 타임아웃 가드 (🟡)

- 등재: 2026-08-30 23:32 (pm 사이클). PR 생성 2026-08-30 20:18 (사람 요청 온디맨드 세션).
- 액션: `gh pr merge 80 --squash --delete-branch` 후 prod 트리 `git pull`. 재기동·재빌드 불필요(ops 스크립트만, 다음 정기 실행부터 발효).
- 근거: 오늘 09:00류 슬롯 결번 재발 방지 — 행(hang)이 launchd 잡 슬롯을 무기한 점유하는 것을 `ops/claude-headless.sh` 타임아웃으로 차단. CI 4잡 SUCCESS · MERGEABLE.
- **주의: AP-20260830-02(e30ab8c)와 충돌** — 둘 다 래퍼 5종(`ops/run-*.sh`)의 같은 claude 호출 블록을 수정하며 PR #80 브랜치는 e30ab8c를 포함하지 않는다. 통합 순서/방향(예: e30ab8c push 먼저 → PR #80을 리베이스해 claude-headless.sh 안으로 session-retry 흡수) 결정 필요.

### AP-20260830-02 — e30ab8c `main` push: 세션 한도 재시도 헬퍼 (🟡)

- 등재: 2026-08-30 23:32 (pm 사이클). 커밋 2026-08-30 20:23 (사람 요청 온디맨드 세션, 로컬 main에만 존재).
- 액션: `git push origin main` (또는 사람이 직접). 오케스트레이터 상태 커밋들이 함께 올라간다.
- 근거: 오늘 09:00 사이클이 세션 한도로 비정상 종료(리셋 27분 뒤 = 재시도만 있었으면 복구) — `ops/lib/session-retry.sh`가 리셋 시각 파싱 후 1회 재시도. 래퍼 5종 적용, 테스트 10케이스 통과. 코드 main push는 🟡 규범이라 자율 push 보류.
- 주의: AP-20260830-01과 충돌 — 위 항목 참조.
