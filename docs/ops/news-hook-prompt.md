...
4. **사실검증** — `Agent` 도구로 별도 서브에이전트를 호출해 다음을 지시한다:
   "다음 URL의 실제 내용을 읽고: (1) 아래 한국어 문장이 그 내용과 사실적으로 일치하는지, (2) 영문 문장이 방금 검증된 한국어 문장과 사실관계상 일치하는 번역인지만 판정해줘(언어 자연스러움은 보지 마). URL: <source_url> / 한국어: <body_ko> / 영문: <body_en> — '일치' 또는 '불일치: <이유>'로만 답해. 두 언어 모두 일치해야 통과."
...
6. **로그·기록**
   - 결과에 따라 report-job-status를 호출한다.
     ...
     - **사실검증 불일치**: `ops/report-job-status.sh news-hook warn "사실검증 불일치로 게시 보류: <사유>"`
