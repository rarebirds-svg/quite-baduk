...
2. **종류·슬러그 선택** — 반드시 아래 절차로 기계적으로 정한다. **이미 게시된 슬러그를 재선택하거나 덮어쓰면 안 된다.** 글로서리·FAQ 한쪽이 굶지 않도록 **비율 균형(most-behind)**으로 종류를 정한다.
   - 두 종류의 기존 슬러그 집합을 확보한다: `ls web/content/glossary/`·`ls web/content/faq/`·`ls docs/ops/content/drafts/`를 **실제로 실행**한다.
   - 각 종류의 **미작성 후보** = 해당 시드(`seed-glossary.yml`/`seed-faq.yml`) 슬러그 중 위 집합에 **없는** 것. (web/content에 파일이 이미 있는 슬러그는 후보에서 제외 — 시드 순서·알파벳 첫 글자만 보고 고르지 말 것.)
   - **종류 결정** — 각 종류의 게시 비율 = `(이미 게시된 시드 슬러그 수) / (시드 총 슬러그 수)`를 계산해, **비율이 낮은(더 뒤처진) 종류**를 고른다. 비율이 같으면 glossary. 단 고른 종류의 미작성 후보가 0개면 다른 종류를 고른다.
   - 고른 종류의 미작성 후보를 **알파벳 순 정렬해 첫 항목**을 슬러그로 선택한다.
   - 두 종류 모두 미작성 후보가 0개면 "처리할 토픽 없음" 로그 + 즉시 종료. **기존 항목을 임의로 개정하지 않는다.**
