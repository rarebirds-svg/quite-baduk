...
- **dev-cycle 세션 동작** — 열린 이슈 중 `bug` 또는 `small`이고 `feature`·
  ...
  작성→수정→통과) → PR 생성(`Closes #N`) → `in-progress` 제거.
- **한정·안전** — 1주기 1개. 항상 PR만 생성하고 머지는 하지 않는다(🟡 — 사람
  게이트). 확신이 안 서거나 테스트가 통과하지 않으면 PR을 만들지 않고 이슈에
  코멘트 + Telegram 에스컬레이션한다. 전용 worktree만 쓰며 prod 트리는 절대 불가.
- 적격 이슈가 없으면 조용히 종료한다.
...
3. dev-cycle이 자명한 시드 `bug` 이슈 1개에 대해 worktree 수정→PR(`Closes #N`)을
   생성한다. `com.inkbaduk.dev-cycle` launchd 작업이 등록된다.
