...
| worktree `node_modules`가 prod로 심볼릭 링크되어 worktree 작업이 prod를 오염 | **각 worktree에 독립 `npm install`** — 심볼릭 링크 금지. AP-20260523-01 장애(`web/node_modules` 비워짐 → inkbaduk.com 502)의 직접 원인이 staging worktree의 심볼릭 링크 였음. 사후 dev-cycle·staging 모두 독립 설치로 전환. |
