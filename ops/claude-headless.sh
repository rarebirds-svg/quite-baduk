...
#
# 배경: 2026-08-30 09:00 orchestrator의 claude 프로세스가 상태 커밋·push까지 마친 뒤
# 종료 직전에 행(hang) — launchd는 같은 라벨의 인스턴스가 살아있으면 다음 슬롯을
# 건너뛰므로, 타임아웃 없는 헤드리스 호출 하나가 잡 슬롯을 무기한 막는다
# (incidents WD-20260830-* 연쇄). macOS에는 GNU timeout이 기본 없어 백그라운드
# 킬러 프로세스로 구현한다.
#
